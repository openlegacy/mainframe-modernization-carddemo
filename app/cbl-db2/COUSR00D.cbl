      ******************************************************************
      * Program     : COUSR00D.CBL
      * Application : CardDemo
      * Type        : CICS COBOL Program
      * Function    : List all users from USERSEC DB2 table
      ******************************************************************
      ...
      *----------------------------------------------------------------*
       ...
       01 WS-VARIABLES.
         05 WS-PGMNAME                 PIC X(08) VALUE 'COUSR00D'.
         05 WS-TRANID                  PIC X(04) VALUE 'ADS0'.
           ...
           IF (CDEMO-CU00-USR-SEL-FLG NOT = SPACES AND LOW-VALUES) AND
              (CDEMO-CU00-USR-SELECTED NOT = SPACES AND LOW-VALUES)
               EVALUATE CDEMO-CU00-USR-SEL-FLG
                   WHEN 'U'
                   WHEN 'u'
                        MOVE 'COUSR02D'   TO CDEMO-TO-PROGRAM
                   ...
                   WHEN 'D'
                   WHEN 'd'
                        MOVE 'COUSR03D'   TO CDEMO-TO-PROGRAM
                        MOVE WS-TRANID    TO CDEMO-FROM-TRANID
                        MOVE WS-PGMNAME   TO CDEMO-FROM-PROGRAM
                        MOVE 0        TO CDEMO-PGM-CONTEXT
                        PERFORM CLOSE-CURSOR-IF-OPEN
      ...
      *----------------------------------------------------------------*
       OPEN-USER-CURSOR.
           ...
           EVALUATE SQLCODE
               ...
               WHEN OTHER
                   MOVE 'Y'     TO WS-ERR-FLG
                   MOVE 'Unable to open user cursor...' TO WS-MESSAGE
                   MOVE -1       TO USRIDINL OF COUSR0AI
                   PERFORM SEND-USRLST-SCREEN
           END-EVALUATE.
      ...
      *----------------------------------------------------------------*
       FETCH-USER-CURSOR.
           ...
           EVALUATE SQLCODE
               ...
               WHEN OTHER
                   MOVE 'Y'     TO WS-ERR-FLG
                   MOVE 'Unable to fetch user data...' TO WS-MESSAGE
                   MOVE -1       TO USRIDINL OF COUSR0AI
                   PERFORM SEND-USRLST-SCREEN
           END-EVALUATE.
